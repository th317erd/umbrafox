...
- {js:meth}`BrowserTestUtils.withNewTab`, allows you to easily run async code
...
Use {js:meth}`TestUtils.topicObserved`:

```js
await TestUtils.topicObserved("sync-pane-loaded");
```

and {js:meth}`BrowserTestUtils.waitForEvent`, respectively:
...
Use {js:meth}`BrowserTestUtils.waitForMutationCondition`.
...
Use {js:meth}`BrowserTestUtils.querySelectorDeep`
