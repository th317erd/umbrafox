...
Gecko-based browser or device which is running a [Marionette]
...
have [pip installed] then run:

```bash
pip install marionette_driver
```

It's highly recommended to use [virtualenv] when installing Marionette to avoid
...
play around with it. Start a [Marionette-enabled instance of Firefox], fire up
