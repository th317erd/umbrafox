...
const TEST_PAGE =
  "https://example.net/browser/browser/base/content/test/browser-protectionsUI/trackerCookiePage.html";
...
add_task(async function testTrackerCookiesSubView() {
  ...
  await SpecialPowers.spawn(tab.linkedBrowser, [], async _ => {
    await content.fetch(
      "https://example.com/browser/browser/base/content/test/browser-protectionsUI/cookieServer.sjs?type=partitioned",
      { credentials: "include" }
    );
  });
  ...
  await SpecialPowers.spawn(tab.linkedBrowser, [], async _ => {
    await content.fetch(
      "https://tracking.example.org/browser/browser/base/content/test/browser-protectionsUI/cookieServer.sjs?type=partitioned",
      { credentials: "include" }
    );
  });
