...
Currently, the installers are built on the [NSIS](http://nsis.sourceforge.net/Main_Page) installer framework. This is a Windows-only framework which compiles scripts written in its custom language (along with some native plugins) into an executable installer.
