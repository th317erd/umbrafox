...
add_task(async function test_contextualsearch_install_prevented_by_policy() {
  // config-engine matches example.org by host but isn't installed (wrong
  // locale), so it is offered as a contextual engine that could be installed.
  ...
  await EnterprisePolicyTesting.setupPolicyEngineWithJson({
    policies: { SearchEngines: { PreventInstalls: true } },
  });

  // The prompt will normally appear on the second time the engine is seen.
  for (let query of ["firstsearch", "secondsearch"]) {
  ...
  Assert.ok(
    !window.gNotificationBox.getNotificationWithValue(NOTIFICATION_VALUE),
    "No install prompt is shown when installing search engines is prevented"
  );
