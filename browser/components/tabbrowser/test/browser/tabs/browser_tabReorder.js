...
add_task(async function () {
  ...
  is(newTab1.index, initialTabsLength, "newTab1 position is correct");
  is(newTab2.index, initialTabsLength + 1, "newTab2 position is correct");
  is(newTab3.index, initialTabsLength + 2, "newTab3 position is correct");

  await dragAndDrop(newTab1, newTab2, false);
  is(gBrowser.tabs.length, initialTabsLength + 3, "tabs are still there");
  is(newTab2.index, initialTabsLength, "newTab2 and newTab1 are swapped");
  is(newTab1.index, initialTabsLength + 1, "newTab1 and newTab2 are swapped");
  is(newTab3.index, initialTabsLength + 2, "newTab3 stays same place");

  await dragAndDrop(newTab2, newTab1, true);
  is(gBrowser.tabs.length, initialTabsLength + 4, "a tab is duplicated");
  is(newTab2.index, initialTabsLength, "newTab2 stays same place");
  is(newTab1.index, initialTabsLength + 1, "newTab1 stays same place");
  is(
    newTab3.index,
    initialTabsLength + 3,
