...
[ESLint](https://eslint.org/) is a popular linter for JavaScript. The ESLint integration also uses
...
- If you don't understand a rule, you can look it in [eslint.org's rule list] for more
...
this is [difficult for ESLint to handle]. Currently this may cause:
...
See the [prettier ignore docs] for more information.
...
- Please do not add this to {searchfox}`eslint-ignores.config.mjs <eslint-ignores.config.mjs>`. Generally
  this can be fixed, if the following tips don't help, please [seek help].
...
[Mark Banner (standard8)](https://people.mozilla.org/s?query=standard8) owns
