...
profile](http://support.mozilla.com/en-US/kb/Managing%20profiles)
where possible.
:::
...
Please follow [these instructions](https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/).
...
a graphical tool, please follow [Capture a minidump in a graphical way](#capture-a-minidump-in-a-graphical-way). If you prefer a command
line tool, please follow [Capture a minidump from the command line](#capture-a-minidump-from-the-command-line).

### Capture a minidump in a graphical way

1. Launch the graphical debugger named WinDbg from the Start Menu. WinDbg
...
2. Connect Firefox to the debugger.
   ...
   1. If Firefox is not already running, open the **"File"** menu on WinDbg
      ...
      it is, please see [How to find the location of firefox.exe](#how-to-find-the-location-of-firefox-exe).
   2. If Firefox is already running and you know which process you want to
      ...
      **"Attach to a Process..."**. In the "Attach to Process" dialog that
      ...
      [Identify a process to attach a debugger to](#identify-a-process-to-attach-a-debugger-to).

3. You should now see a "Command" text window with debug output at the
...
4. When the program is about to crash, WinDbg will spit out more data,
...
5. Ask in the relevant bug or thread how best to share this very large
   file!
...
of firefox.exe](#how-to-find-the-location-of-firefox-exe).
...
please see [Identify a process to attach a debugger to](#identify-a-process-to-attach-a-debugger-to).
...
instructions](http://support.microsoft.com/kb/931673) to capture a
...
The second way is to use [Process Explorer](https://docs.microsoft.com/en-us/sysinternals/downloads/process-explorer),
