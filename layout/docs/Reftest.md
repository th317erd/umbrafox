...
Layout Engine Visual Tests (reftest)
L. David Baron \<<mailto:dbaron@dbaron.org>>, Mozilla Corporation
July 19, 2006
...
1. `<failure-type>` (optional) is one of the following:
...
2. `<preference>` (optional) is a string of the form
...
3. `<http>`, if present, is one of the strings (sans quotes) "HTTP" or
...
4. `<type>` is one of the following:
...
5. `<url>` is either a relative file path or an absolute URL for the
   test page

6. `<url_ref>` is either a relative file path or an absolute URL for
   the reference page
