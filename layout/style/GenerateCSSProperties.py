...
"""
    ...
    for prop in properties.all_properties_and_aliases():
        flags = " | ".join("CSSPropFlags::{}".format(flag) for flag in cpp_flags(prop))
        if not flags:
...
interface """
    ...
    for p in properties.all_properties_and_aliases():
        ...
        extendedAttrs = [
            "BindingTemplate=(%s, NonCustomCSSPropertyId, eCSSProperty_%s)"
            % (bindingTemplate, propId),
            "CEReactions",
            "SetterThrows",
...
def gen_position_try_descriptors_webidl(output):
    return gen_webidl(
        output, "position-try", "CSSPositionTryDescriptors", "CSSPositionTryDescriptor"
    )
...
""")
    for d in properties.font_face_descriptors:
        ...
        extendedAttrs = [
            "BindingTemplate=(CSSFontFaceDesc, FontFaceDescriptorId, %s)"
            % d.camel_case,
            "SetterThrows",
        ]
        if d.gecko_pref:
...
def gen_non_custom_css_property_id(output, template):
    ...
    shorthand_count = property_ids[
        len(properties.longhands) + len(properties.shorthands)
    ]
    ...
    output.write(
        template.substitute({
            ...
            "font_face_descriptors": "\n".join(
                "  {},".format(d.camel_case) for d in properties.font_face_descriptors
            ),
