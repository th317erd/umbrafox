...
myst:
  substitutions:
    br: |-
      ...
      <br/>```
    image1: |-
      ```{image} blocked_nw_icon.png
      :alt: Red circle with a diagonal slash
      :width: 20```
    image2: |-
      ```{image} nwmon-turtle-tooltip.png
      :alt: Screenshot of a network request with a turtle icon, and a tooltip explaining
      :  the problem
      :class: border```
    image3: |-
      ```{image} https.svg
      :width: 20```
    image4: |-
      ```{image} https-weak.svg
      :width: 20```
    image5: |-
      ```{image} https-failed.svg
      :width: 20```
    image6: |-
      ```{image} http.svg
      :width: 20```
...
:alt: Screenshot of the context menu for selecting columns to display in the Network
:  monitor
:class: center
```
...
- **Transferred**: The number of bytes that were actually transferred to load the resource, or a message about why the resource was not transferred. A number value is less than **Size** if the resource was compressed.
  ...
  - If the resource was fetched from a [service worker](https://developer.mozilla.org/en-US/docs/Web/API/Service_Worker_API) cache, then this cell displays "service worker".
...
(network-monitor-request-list-security-icons)=
...
- The blue line marks the point at which thepage's [DOMContentLoaded](https://developer.mozilla.org/en-US/docs/Web/API/Window/DOMContentLoaded_event) event is triggered.
- The red line marks the point at which the page's [load](https://developer.mozilla.org/en-US/docs/Web/API/Window/load_event) event is triggered.

(network-monitor-blocking-specific-urls)=
...
:alt: Screen shot of the Blocking panel, with arrows indicating the panel and the Request
:  Blocking toolbar icon
:class: center
```
...
(request-list-filtering-by-properties)=

(request-list-requst-list-cause-column)=
...
.. list-table::
  ...
  * - ``mime-type``
    ...
    - ``mime-type:text/html``  ``mime-type:image/png``  ``mime-type:application/javascript``
  ...
  * - ``has-response-header``
    ...
    - ``has-response-header:cache-control``  ``has-response-header:X-Firefox-Spdy``
...
:alt: Screenshot of the Network monitor, with the request search sidebar displayed,
:  and arrows indicating the search toolbar icon and the search box.
:class: border
```
...
:alt: Screenshot of the search panel, with "newsletter" as the search string, and callouts
:  for the expanded results, and corresponding items displayed in the request list
:  and headers tab.
:class: border
```
...
(network-monitor-request-list-edit-and-resend)=
...
.. list-table::
  ...
  * - Block URL
    - Blocks the selected URL for future requests. See :ref:`Blocking a specific URL from the Request List <network-monitor-blocking-specific-urls>`.
...
.. list-table::
  ...
  * - ``-H``
    - One for each request header.  If the "Accept-Encoding" header is present, the cURL command includes ``--compressed`` instead of ``-H "Accept-Encoding: gzip, deflate"``. This means that the response will be automatically decompressed.
