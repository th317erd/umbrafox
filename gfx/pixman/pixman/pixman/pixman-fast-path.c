...
{									      \
    ...
    PIXMAN_IMAGE_GET_LINE (dest_image, dest_x, dest_y, pix_type,              \
			   dst_stride, dst_line, 1);                          \
    /* Round in pixman_fixed_48_16_t, to avoid overflows. Since               \
     * matrix values are pixman_fixed_16_16_t, even if the value rounds       \
     * up, it will still represent a value < 65537, and fit in an int.  */    \
