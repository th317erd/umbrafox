...
TEST_F(NimbusTelemetryFixture, NimbusFeaturesTelemetry) {
  constexpr auto prefName =
      "nimbus.syncdatastore.testPlatformExposureFeature"_ns;
  ...
  ASSERT_EQ(
      NimbusFeatures::RecordExposureEvent("testPlatformExposureFeature"_ns),
      NS_ERROR_UNEXPECTED)
  ...
  ASSERT_EQ(NimbusFeatures::RecordExposureEvent(
                "testPlatformExposureFeature"_ns, true),
            NS_ERROR_ABORT)
      << "Should abort because we've sent exposure and aOnce is true";

  ASSERT_EQ(Preferences::ClearUser(prefName.get()), NS_OK);
  ASSERT_EQ(
      NimbusFeatures::RecordExposureEvent("testPlatformExposureFeature"_ns),
      NS_ERROR_UNEXPECTED)
      << "Should fail because we longer have an experiment";
