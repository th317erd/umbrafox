...
extern PRStatus _PR_NT_MakeSecurityDescriptorACL(PRIntn mode,
                                                 DWORD accessTable[],
...
/* --- Lock stuff --- */
#define _MD_NEW_LOCK(lock) \
    (InitializeCriticalSection(&((lock)->mutex)), PR_SUCCESS)
...
#define _MD_TEST_AND_LOCK(lock) \
    (TryEnterCriticalSection(&((lock)->mutex)) == FALSE)
...
#define _MD_GET_INTSOFF() \
    (_pr_use_static_tls ? _pr_ints_off : (PRUintn)TlsGetValue(_pr_intsOffIndex))
