...
#define _PT_THREAD_BLOCK_INTERRUPT(thr) (thr->interrupt_blocked = 1)
#define _PT_THREAD_UNBLOCK_INTERRUPT(thr) (thr->interrupt_blocked = 0)

#define _PT_IS_GCABLE_THREAD(thr) ((thr)->state & PT_THREAD_GCABLE)

/*
** Possible values for thread's suspend field
** Note that the first two can be the same as they are really mutually
* exclusive,
...
NSPR_API(void)
PT_FPrintStats(PRFileDesc* fd, const char* msg);
...
#define _PR_INTSOFF(_is)                        \
    PR_BEGIN_MACRO(_is) = _PR_MD_GET_INTSOFF(); \
...
#define _PR_DEL_JOINQ(_thread) PR_REMOVE_LINK(&(_thread)->links);

#define _PR_ADD_SUSPENDQ(_thread, _cpu) \
    PR_APPEND_LINK(&(_thread)->links, &_PR_SUSPENDQ(_cpu));

#define _PR_DEL_SUSPENDQ(_thread) PR_REMOVE_LINK(&(_thread)->links);
...
extern PRStatus _PR_WaitCondVar(PRThread* thread, PRCondVar* cvar, PRLock* lock,
                                PRIntervalTime timeout);
extern void _PR_NotifyCondVar(PRCondVar* cvar, PRThread* me);
extern PRUint32 _PR_CondVarToString(PRCondVar* cvar, char* buf,
                                    PRUint32 buflen);

NSPR_API(void)
_PR_Notify(PRMonitor* mon, PRBool all, PRBool sticky);
...
** dependent value.  This is only for standard user threads.  For custom
* threads,
...
extern void _PR_SetThreadPriority(PRThread* thread, PRThreadPriority priority);
...
extern PRThread* _PRI_AttachThread(PRThreadType type, PRThreadPriority priority,
...
NSPR_API(PRStatus)
_PR_MD_INIT_ATTACHED_THREAD(PRThread* thread);
...
#define _PR_MD_CREATE_PRIMORDIAL_USER_THREAD \
    _MD_CREATE_PRIMORDIAL_USER_THREAD

extern PRThread* _PR_MD_CREATE_USER_THREAD(PRUint32 stacksize,
                                           void (*start)(void*), void* arg);
#define _PR_MD_CREATE_USER_THREAD _MD_CREATE_USER_THREAD
#endif

extern PRStatus _PR_MD_CREATE_THREAD(PRThread* thread, void (*start)(void*),
...
NSPR_API(void)
_PR_MD_SUSPENDALL(void);
#define _PR_MD_SUSPENDALL _MD_SUSPENDALL

NSPR_API(void)
_PR_MD_RESUMEALL(void);
#define _PR_MD_RESUMEALL _MD_RESUMEALL

extern void _PR_MD_INIT_CONTEXT(PRThread* thread, char* top,
                                void (*start)(void), PRBool* status);
...
extern PRStatus _PR_MD_ALLOC_SEGMENT(PRSegment* seg, PRUint32 size,
                                     void* vaddr);
...
extern PRInt32 _PR_MD_WRITEV(PRFileDesc* fd, const struct PRIOVec* iov,
...
extern PROsfd _PR_MD_OPEN_FILE_UTF16(const PRUnichar* name, PRIntn osflags,
                                     PRIntn mode);
...
extern PRInt32 _PR_MD_GETFILEINFO64_UTF16(const PRUnichar* fn,
                                          PRFileInfo64* info);
...
extern PRInt32 _PR_MD_CONNECT(PRFileDesc* fd, const PRNetAddr* addr,
                              PRUint32 addrlen, PRIntervalTime timeout);
#define _PR_MD_CONNECT _MD_CONNECT

extern PROsfd _PR_MD_ACCEPT(PRFileDesc* fd, PRNetAddr* addr, PRUint32* addrlen,
                            PRIntervalTime timeout);
#define _PR_MD_ACCEPT _MD_ACCEPT

extern PRInt32 _PR_MD_BIND(PRFileDesc* fd, const PRNetAddr* addr,
                           PRUint32 addrlen);
...
extern PRInt32 _PR_MD_SEND(PRFileDesc* fd, const void* buf, PRInt32 amount,
                           PRIntn flags, PRIntervalTime timeout);
...
extern PRInt32 _PR_MD_SENDFILE(PRFileDesc* sock, PRSendFileData* sfd,
                               PRInt32 flags, PRIntervalTime timeout);
#define _PR_MD_SENDFILE _MD_SENDFILE

extern PRStatus _PR_MD_GETSOCKNAME(PRFileDesc* fd, PRNetAddr* addr,
                                   PRUint32* addrlen);
#define _PR_MD_GETSOCKNAME _MD_GETSOCKNAME

extern PRStatus _PR_MD_GETPEERNAME(PRFileDesc* fd, PRNetAddr* addr,
                                   PRUint32* addrlen);
#define _PR_MD_GETPEERNAME _MD_GETPEERNAME

extern PRStatus _PR_MD_GETSOCKOPT(PRFileDesc* fd, PRInt32 level,
                                  PRInt32 optname, char* optval,
                                  PRInt32* optlen);
#define _PR_MD_GETSOCKOPT _MD_GETSOCKOPT

extern PRStatus _PR_MD_SETSOCKOPT(PRFileDesc* fd, PRInt32 level,
                                  PRInt32 optname, const char* optval,
                                  PRInt32 optlen);
#define _PR_MD_SETSOCKOPT _MD_SETSOCKOPT

extern PRStatus PR_CALLBACK _PR_SocketGetSocketOption(PRFileDesc* fd,
                                                      PRSocketOptionData* data);

extern PRStatus PR_CALLBACK
_PR_SocketSetSocketOption(PRFileDesc* fd, const PRSocketOptionData* data);

extern PRInt32 _PR_MD_RECVFROM(PRFileDesc* fd, void* buf, PRInt32 amount,
                               PRIntn flags, PRNetAddr* addr, PRUint32* addrlen,
                               PRIntervalTime timeout);
#define _PR_MD_RECVFROM _MD_RECVFROM

extern PRInt32 _PR_MD_SENDTO(PRFileDesc* fd, const void* buf, PRInt32 amount,
                             PRIntn flags, const PRNetAddr* addr,
                             PRUint32 addrlen, PRIntervalTime timeout);
#define _PR_MD_SENDTO _MD_SENDTO

#if defined(_WIN64) && defined(WIN95)
extern PRInt32 _PR_MD_TCPSENDTO(PRFileDesc* fd, const void* buf, PRInt32 amount,
                                PRIntn flags, const PRNetAddr* addr,
                                PRUint32 addrlen, PRIntervalTime timeout);
...
extern PROffset32 _PR_MD_LSEEK(PRFileDesc* fd, PROffset32 offset,
                               PRSeekWhence whence);
#define _PR_MD_LSEEK _MD_LSEEK

extern PROffset64 _PR_MD_LSEEK64(PRFileDesc* fd, PROffset64 offset,
                                 PRSeekWhence whence);
...
extern PRInt32 _PR_MD_GETOPENFILEINFO64(const PRFileDesc* fd,
                                        PRFileInfo64* info);
...
#else
#define PR_NETADDR_SIZE(_addr)                                 \
    ((_addr)->raw.family == PR_AF_INET ? sizeof((_addr)->inet) \
                                       : sizeof(struct _md_sockaddr_in6))
...
#define PR_NETADDR_SIZE(_addr)                                         \
    ((_addr)->raw.family == PR_AF_INET                                 \
         ? sizeof((_addr)->inet)                                       \
         : ((_addr)->raw.family == PR_AF_INET6 ? sizeof((_addr)->ipv6) \
                                               : sizeof((_addr)->local)))
#else
#define PR_NETADDR_SIZE(_addr)                                 \
    ((_addr)->raw.family == PR_AF_INET ? sizeof((_addr)->inet) \
...
extern PRStatus _PR_MapOptionName(PRSockOption optname, PRInt32* level,
                                  PRInt32* name);
extern void _PR_InitThreads(PRThreadType type, PRThreadPriority priority,
                            PRUintn maxPTDs);
...
struct PRCondVar {
    ...
    sem_id
        handshakeSem;    /* the lock for 'notify'-threads waiting for confirmation */
...
#if !defined(_PR_NO_PREEMPT) && defined(XP_UNIX) && !defined(_PR_PTHREADS) && \
    !defined(_PR_GLOBAL_THREADS_ONLY) && !defined(PURIFY) &&                  \
    !defined(DARWIN) && !defined(QNX)
...
extern PRProcess* _PR_MD_CREATE_PROCESS(const char* path, char* const* argv,
                                        char* const* envp,
...
extern void* _PR_MD_MEM_MAP(PRFileMap* fmap, PROffset64 offset, PRUint32 len);
...
extern PRStatus _PR_MD_SYNC_MEM_MAP(PRFileDesc* fd, void* addr, PRUint32 len);
 ...
 */
extern PRStatus _PR_MakeNativeIPCName(const char* name, /* NSPR IPC name */
...
extern PRSize _pr_CopyLowBits(void* dest, PRSize dstlen, void* src,
                              PRSize srclen);
