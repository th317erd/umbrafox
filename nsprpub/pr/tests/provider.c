...
typedef enum CSState_e { cs_init,
                         cs_run,
                         cs_stop,
                         cs_exit } CSState_t;
...
static void
_MY_Assert(const char* s, const char* file, PRIntn ln)
{
    PL_PrintError(NULL);
    PR_Assert(s, file, ln);
} /* _MY_Assert */
...
static void
TimeOfDayMessage(const char* msg, PRThread* me)
{
    char buffer[100];
    PRExplodedTime tod;
...
static PRStatus
ProcessRequest(PRFileDesc* fd, CSServer_t* server)
{
    PRStatus drv, rv;
    char buffer[1024];
...
static PRStatus
NewThread(StartFn start, void* arg, PRThreadPriority prio,
          PRThreadState state)
{
    PRStatus rv;
...
static PRStatus
CreateWorker(CSServer_t* server, CSPool_t* pool)
{
    PRStatus rv;
...
static void
WaitForCompletion(PRIntn execution)
{
    while (execution > 0) {
