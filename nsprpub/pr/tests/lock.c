...
static PRIntervalTime
NonContentiousLock(PRUint32 loops)
{
    PRLock* ml = NULL;
    ml = PR_NewLock();
    while (loops-- > 0) {
...
static PRIntervalTime
ContentiousLock(PRUint32 loops)
{
    PRStatus status;
...
static PRIntervalTime
NonContentiousMonitor(PRUint32 loops)
{
    PRMonitor* ml = NULL;
    ml = PR_NewMonitor();
    while (loops-- > 0) {
...
static PRIntervalTime
ReentrantMonitor(PRUint32 loops)
{
    PRStatus status;
    PRThread* thread;
...
static PRUint32
ContentiousMonitor(PRUint32 loops)
{
    PRStatus status;
    PRThread* thread = NULL;
...
*/
static PRIntervalTime
NonContentiousCMonitor(PRUint32 loops)
{
    MonitorContentious_t contention;
    while (loops-- > 0) {
...
static PRIntervalTime
ContentiousCMonitor(PRUint32 loops)
{
    PRStatus status;
    PRThread* thread = NULL;
    MonitorContentious_t* contention;
...
static PRIntervalTime
Test(const char* msg, PRUint32 (*test)(PRUint32 loops),
