...
void
Test_Assert(const char* msg, const char* file, PRIntn line)
{
    failed_already = 1;
    if (debug_mode) {
...
#define TEST_ASSERT(expr) \
    if (!(expr))          \
    Test_Assert(#expr, __FILE__, __LINE__)

#ifdef WINNT
#define CALLBACK_MAGIC 0x12345678

void
timeout_callback(void* magic)
{
...
void
AcceptUpdatedTest(void)
{
    RunTest(ACCEPT_NORMAL, CLIENT_NORMAL);
}
void
AcceptNotUpdatedTest(void)
{
