...
static PRIntn
Error(const char* msg, const char* filename)
{
...
static void
Verbose(Verbosity level, const char* msg, const char* file,
        PRIntn line)
{
    if (level <= verbose) {
...
static void
PrintInfo(PRFileInfo64* info, const char* filename)
{
    PRExplodedTime tm;
