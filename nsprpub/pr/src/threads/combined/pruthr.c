...
*/
static void
_PR_InitializeNativeStack(PRThreadStack* ts)
{
    if (ts && (ts->stackTop == 0)) {
...
void
_PR_NotifyJoinWaiters(PRThread* thread)
{
    /*
 ...
 */
static void
_PR_InitializeRecycledThread(PRThread* thread)
{
    /*
...
PRStatus
_PR_RecycleThread(PRThread* thread)
{
...
void
_PR_SetThreadPriority(PRThread* thread, PRThreadPriority newPri)
{
...
static PRThread*
get_thread(_PRCPU* cpu, PRBool* wakeup_cpus)
{
    PRThread* thread;
    PRIntn pri;
    PRUint32 r;
...
*/
PRThread*
_PRI_AttachThread(PRThreadType type, PRThreadPriority priority,
