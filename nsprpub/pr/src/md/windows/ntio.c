...
static void
_native_thread_io_nowait(PRThread* thread, int rv, int bytes)
{
    int rc;
...
void
_PR_MD_UPDATE_ACCEPT_CONTEXT(PROsfd accept_sock, PROsfd listen_sock)
{
...
PRInt32
_PR_MD_FAST_ACCEPT_READ(PRFileDesc* sd, PROsfd* newSock,
...
PRInt32
_PR_MD_SENDFILE(PRFileDesc* sock, PRSendFileData* sfd, PRInt32 flags,
                PRIntervalTime timeout)
{
    PRThread* me = _PR_MD_CURRENT_THREAD();
    PRInt32 tflags;
...
PRInt32
_PR_MD_SEND(PRFileDesc* fd, const void* buf, PRInt32 amount,
...
PRInt32
_PR_MD_SENDTO(PRFileDesc* fd, const void* buf, PRInt32 amount,
...
PROffset32
_PR_MD_LSEEK(PRFileDesc* fd, PROffset32 offset,
             PRSeekWhence whence)
{
    DWORD moveMethod;
    PROffset32 rv;
...
PROffset64
_PR_MD_LSEEK64(PRFileDesc* fd, PROffset64 offset,
               PRSeekWhence whence)
{
    DWORD moveMethod;
    LARGE_INTEGER li;
...
IsRootDirectory(char* fn, size_t buflen)
{
    ...
    if (isalpha((unsigned char)fn[0]) && fn[1] == ':' && _PR_IS_SLASH(fn[2]) &&
        fn[3] == '\0') {
...
static PROsfd
_nt_nonblock_accept(PRFileDesc* fd, struct sockaddr* addr,
...
static PRInt32
_nt_nonblock_connect(PRFileDesc* fd, struct sockaddr* addr,
                     int addrlen, PRIntervalTime timeout)
{
    PROsfd osfd = fd->secret->md.osfd;
    PRInt32 rv;
...
static PRInt32
_nt_nonblock_recv(PRFileDesc* fd, char* buf, int len, int flags,
                  PRIntervalTime timeout)
{
    PROsfd osfd = fd->secret->md.osfd;
    PRInt32 rv, err;
...
static PRInt32
_nt_nonblock_send(PRFileDesc* fd, char* buf, int len,
                  PRIntervalTime timeout)
{
    PROsfd osfd = fd->secret->md.osfd;
    PRInt32 rv, err;
...
static PRInt32
_nt_nonblock_writev(PRFileDesc* fd, const PRIOVec* iov, int size,
                    PRIntervalTime timeout)
{
    int index;
    int sent = 0;
...
static PRInt32
_nt_nonblock_sendto(PRFileDesc* fd, const char* buf, int len,
...
static PRInt32
_nt_nonblock_recvfrom(PRFileDesc* fd, char* buf, int len,
...
static void
pt_InsertTimedInternal(pt_Continuation* op)
{
    PRInt32 delta = 0;
 ...
 */
static pt_Continuation*
pt_FinishTimedInternal(pt_Continuation* op)
{
...
static PRInt32
pt_SendTo(SOCKET osfd, const void* buf, PRInt32 amount,
          PRInt32 flags, const PRNetAddr* addr, PRIntn addrlen,
          PRIntervalTime timeout)
{
    PRInt32 bytes = -1, err;
...
static PRInt32
pt_RecvFrom(SOCKET osfd, void* buf, PRInt32 amount,
            PRInt32 flags, PRNetAddr* addr, PRIntn* addr_len,
            PRIntervalTime timeout)
{
    PRInt32 bytes = -1, err;
