 ...
 */
static int
assembleCmdLine(char* const* argv, char** cmdLine)
{
    char* const* arg;
    char *p, *q;
    size_t cmdLineSize;
 ...
 */
static int
assembleEnvBlock(char** envp, char** envBlock)
{
    char* p;
    char* q;
    char** env;
...
PRProcess*
_PR_CreateWindowsProcess(const char* path, char* const* argv,
                         char* const* envp,
...
PRStatus
_PR_DetachWindowsProcess(PRProcess* process)
{
 ...
 */
PRStatus
_PR_WaitWindowsProcess(PRProcess* process, PRInt32* exitCode)
{
    DWORD dwRetVal;
...
PRStatus
_PR_KillWindowsProcess(PRProcess* process)
{
    /*
...
PRStatus
_MD_WindowsGetReleaseName(char* name, PRUint32 namelen)
{
    OSVERSIONINFO osvi;
...
void*
_MD_MemMap(PRFileMap* fmap, PROffset64 offset, PRUint32 len)
{
    DWORD dwHi, dwLo;
    void* addr;
...
#pragma warning(disable : 4035)
PRInt32
_PR_MD_ATOMIC_ADD(PRInt32* intp, PRInt32 val)
{
#if defined(__GNUC__)
    PRInt32 result;
