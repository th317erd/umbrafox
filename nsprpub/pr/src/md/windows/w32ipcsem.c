...
#ifdef WINCE
static HANDLE
OpenSemaphore(DWORD inDesiredAccess, BOOL inInheritHandle,
              const char* inName)
{
    HANDLE retval = NULL;
...
static DWORD
FiberSafeWaitForSingleObject(HANDLE hHandle,
...
PRSem*
_PR_MD_OPEN_SEMAPHORE(const char* osname, PRIntn flags, PRIntn mode,
                      PRUintn value)
{
    PRSem* sem;
...
PRStatus
_PR_MD_WAIT_SEMAPHORE(PRSem* sem)
{
    DWORD rv;
...
PRStatus
_PR_MD_POST_SEMAPHORE(PRSem* sem)
{
...
PRStatus
_PR_MD_CLOSE_SEMAPHORE(PRSem* sem)
{
