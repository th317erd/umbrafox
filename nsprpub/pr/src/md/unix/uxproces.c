...
static PRProcess*
ForkAndExec(const char* path, char* const* argv,
...
PRProcess*
_MD_CreateUnixProcess(const char* path, char* const* argv,
                      char* const* envp, const PRProcessAttr* attr)
{
...
static void
WaitPidDaemonThread(void* unused)
{
    pid_t pid;
    int status;
...
PRStatus
_MD_DetachUnixProcess(PRProcess* process)
{
...
PRStatus
_MD_WaitUnixProcess(PRProcess* process, PRInt32* exitCode)
{
    pr_PidRecord* pRec;
    PRStatus retVal = PR_SUCCESS;
...
PRStatus
_MD_KillUnixProcess(PRProcess* process)
{
    PRErrorCode prerror;
    PRInt32 oserror;
