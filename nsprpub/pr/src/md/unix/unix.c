...
#elif defined(SOLARIS) || defined(AIX4_1) || defined(LINUX) || \
...
void
_MD_unix_init_running_cpu(_PRCPU* cpu)
{
...
PRStatus
_MD_open_dir(_MDDir* d, const char* name)
{
    int err;
...
char*
_MD_read_dir(_MDDir* d, PRIntn flags)
{
    struct dirent* de;
    int err;
...
PRInt32
_MD_access(const char* name, PRAccessHow how)
{
    PRInt32 rv, err;
    int amode;
...
PRInt64
_MD_socketavailable64(PRFileDesc* fd)
{
    PRInt64 result;
...
static PRInt32
socket_io_wait(PRInt32 osfd, PRInt32 fd_type,
               PRIntervalTime timeout)
{
    PRInt32 rv = -1;
    struct timeval tv;
...
static PRInt32
local_io_wait(PRInt32 osfd, PRInt32 wait_flag,
              PRIntervalTime timeout)
{
    _PRUnixPollDesc pd;
    PRInt32 rv;
...
PRInt32
_MD_sendto(PRFileDesc* fd, const void* buf, PRInt32 amount,
...
PRStatus
_MD_getsockopt(PRFileDesc* fd, PRInt32 level, PRInt32 optname,
...
PRStatus
_MD_setsockopt(PRFileDesc* fd, PRInt32 level, PRInt32 optname,
...
*/
void
_MD_PauseCPU(PRIntervalTime ticks)
{
    PRThread* me = _MD_CURRENT_THREAD();
#ifdef _PR_USE_POLL
    int timeout;
...
PROffset32
_MD_lseek(PRFileDesc* fd, PROffset32 offset, PRSeekWhence whence)
{
    PROffset32 rv, where;
...
*/
static void
_MD_set_fileinfo_times(const struct stat* sb, PRFileInfo* info)
{
    PRInt64 us, s2us;
...
static void
_MD_set_fileinfo64_times(const _MDStat64* sb, PRFileInfo64* info)
{
    PRInt64 us, s2us;
...
static int
_MD_convert_stat_to_fileinfo(const struct stat* sb,
                             PRFileInfo* info)
{
...
static int
_MD_convert_stat64_to_fileinfo64(const _MDStat64* sb,
                                 PRFileInfo64* info)
{
    if (S_IFREG & sb->st_mode) {
        info->type = PR_FILE_FILE;
...
static PRIntn
_MD_solaris25_fstat64(PRIntn osfd, _MDStat64* buf)
{
    PRInt32 rv;
    struct stat sb;
...
static PRIntn
_MD_solaris25_stat64(const char* fn, _MDStat64* buf)
{
    PRInt32 rv;
    struct stat sb;
...
static PROffset64
_MD_Unix_lseek64(PRIntn osfd, PROffset64 offset,
                 PRIntn whence)
{
    PRUint64 maxoff;
...
static void*
_MD_Unix_mmap64(void* addr, PRSize len, PRIntn prot, PRIntn flags,
 ...
 */
PRInt32
_PR_WaitForMultipleFDs(_PRUnixPollDesc* unixpds, PRInt32 pdcnt,
                       PRIntervalTime timeout)
{
    PRPollQueue pq;
    PRIntn is;
 ...
 */
void
_PR_Unblock_IO_Wait(PRThread* thr)
{
...
int
_MD_unix_get_nonblocking_connect_error(int osfd)
{
#if defined(NTO)
...
PRStatus
_MD_getsysinfo(PRSysInfo cmd, char* name, PRUint32 namelen)
{
    struct utsname info;
...
PRStatus
_MD_CreateFileMap(PRFileMap* fmap, PRInt64 size)
{
    PRFileInfo info;
    PRUint32 sz;
...
void*
_MD_MemMap(PRFileMap* fmap, PRInt64 offset, PRUint32 len)
{
    PRInt32 off;
    void* addr;
...
PRStatus
_MD_MemUnmap(void* addr, PRUint32 len)
{
...
PRStatus
_MD_CloseFileMap(PRFileMap* fmap)
{
...
PRStatus
_MD_SyncMemMap(PRFileDesc* fd, void* addr, PRUint32 len)
{
...
int
poll(struct pollfd* filedes, unsigned long nfds, int timeout)
{
    int i;
    int rv;
    int maxfd;
