...
PRBool
IsValidNetAddr(const PRNetAddr* addr)
{
...
static PRBool
IsValidNetAddrLen(const PRNetAddr* addr, PRInt32 addr_len)
{
    /*
...
PR_IMPLEMENT(void)
PT_FPrintStats(PRFileDesc* debug_out, const char* msg)
{
    PTDebug stats;
    char buffer[100];
    PRExplodedTime tod;
...
static void
pt_poll_now(pt_Continuation* op)
{
    PRInt32 msecs;
...
static PRIntn
pt_Continue(pt_Continuation* op)
{
...
/*****************************************************************************/
static PRBool
pt_connect_cont(pt_Continuation* op, PRInt16 revents)
{
...
static PRBool
pt_accept_cont(pt_Continuation* op, PRInt16 revents)
{
    op->syserrno = 0;
...
static PRBool
pt_read_cont(pt_Continuation* op, PRInt16 revents)
{
    /*
...
static PRBool
pt_recv_cont(pt_Continuation* op, PRInt16 revents)
{
    /*
     * Any number of bytes will complete the operation. It need
     * not (and probably will not) satisfy the request. The only
...
static PRBool
pt_send_cont(pt_Continuation* op, PRInt16 revents)
{
    PRIntn bytes;
#if defined(SOLARIS)
...
static PRBool
pt_writev_cont(pt_Continuation* op, PRInt16 revents)
{
    PRIntn bytes;
...
static PRBool
pt_sendto_cont(pt_Continuation* op, PRInt16 revents)
{
    PRIntn bytes =
...
static PRBool
pt_recvfrom_cont(pt_Continuation* op, PRInt16 revents)
{
...
#ifdef AIX
static PRBool
pt_aix_sendfile_cont(pt_Continuation* op, PRInt16 revents)
{
...
#ifdef HPUX11
static PRBool
pt_hpux_sendfile_cont(pt_Continuation* op, PRInt16 revents)
{
...
#ifdef SOLARIS
static PRBool
pt_solaris_sendfile_cont(pt_Continuation* op, PRInt16 revents)
{
...
#ifdef LINUX
static PRBool
pt_linux_sendfile_cont(pt_Continuation* op, PRInt16 revents)
{
    ssize_t rv;
    off_t oldoffset;
...
static void
pt_MapError(void (*mapper)(PRIntn), PRIntn syserrno)
{
    switch (syserrno) {
        case EINTR:
            PR_SetError(PR_PENDING_INTERRUPT_ERROR, 0);
...
static PRInt32
pt_Read(PRFileDesc* fd, void* buf, PRInt32 amount)
{
...
static PRInt32
pt_Write(PRFileDesc* fd, const void* buf, PRInt32 amount)
{
    PRInt32 syserrno, bytes = -1;
...
static PRInt32
pt_Writev(PRFileDesc* fd, const PRIOVec* iov, PRInt32 iov_len,
...
static PRInt32
pt_Seek(PRFileDesc* fd, PRInt32 offset, PRSeekWhence whence)
{
    return _PR_MD_LSEEK(fd, offset, whence);
} /* pt_Seek */

static PRInt64
pt_Seek64(PRFileDesc* fd, PRInt64 offset, PRSeekWhence whence)
{
    return _PR_MD_LSEEK64(fd, offset, whence);
} /* pt_Seek64 */

static PRInt32
pt_Available_f(PRFileDesc* fd)
{
...
static PRInt64
pt_Available64_f(PRFileDesc* fd)
{
...
static PRInt64
pt_Available64_s(PRFileDesc* fd)
{
    PRInt64 rv;
...
static PRStatus
pt_FileInfo(PRFileDesc* fd, PRFileInfo* info)
{
...
static PRStatus
pt_FileInfo64(PRFileDesc* fd, PRFileInfo64* info)
{
...
static PRStatus
pt_Synch(PRFileDesc* fd)
{
    return (NULL == fd) ? PR_FAILURE : PR_SUCCESS;
} /* pt_Synch */

static PRStatus
pt_Fsync(PRFileDesc* fd)
{
    PRIntn rv = -1;
    if (pt_TestAbort()) {
        return PR_FAILURE;
...
static PRStatus
pt_Connect(PRFileDesc* fd, const PRNetAddr* addr,
           PRIntervalTime timeout)
{
...
static PRStatus
pt_ConnectContinue(PRFileDesc* fd, PRInt16 out_flags)
{
    int err;
    PRInt32 osfd;
...
static PRFileDesc*
pt_Accept(PRFileDesc* fd, PRNetAddr* addr,
          PRIntervalTime timeout)
{
...
static PRStatus
pt_Bind(PRFileDesc* fd, const PRNetAddr* addr)
{
    PRIntn rv;
    pt_SockLen addr_len;
    const PRNetAddr* addrp = addr;
...
static PRStatus
pt_Listen(PRFileDesc* fd, PRIntn backlog)
{
    PRIntn rv;
...
static PRStatus
pt_Shutdown(PRFileDesc* fd, PRIntn how)
{
    PRIntn rv = -1;
    if (pt_TestAbort()) {
        return PR_FAILURE;
...
static PRInt16
pt_Poll(PRFileDesc* fd, PRInt16 in_flags, PRInt16* out_flags)
{
    *out_flags = 0;
    return in_flags;
} /* pt_Poll */
...
static PRInt32
pt_Recv(PRFileDesc* fd, void* buf, PRInt32 amount, PRIntn flags,
        PRIntervalTime timeout)
{
    PRInt32 syserrno, bytes = -1;
    PRIntn osflags;
...
static PRInt32
pt_SocketRead(PRFileDesc* fd, void* buf, PRInt32 amount)
{
    return pt_Recv(fd, buf, amount, 0, PR_INTERVAL_NO_TIMEOUT);
} /* pt_SocketRead */

static PRInt32
pt_Send(PRFileDesc* fd, const void* buf, PRInt32 amount,
...
static PRInt32
pt_SocketWrite(PRFileDesc* fd, const void* buf, PRInt32 amount)
{
    return pt_Send(fd, buf, amount, 0, PR_INTERVAL_NO_TIMEOUT);
} /* pt_SocketWrite */

static PRInt32
pt_SendTo(PRFileDesc* fd, const void* buf, PRInt32 amount,
...
static PRInt32
pt_TCP_SendTo(PRFileDesc* fd, const void* buf, PRInt32 amount,
              PRIntn flags, const PRNetAddr* addr,
              PRIntervalTime timeout)
{
...
static PRInt32
pt_RecvFrom(PRFileDesc* fd, void* buf, PRInt32 amount,
...
static void
pt_aix_sendfile_init_routine(void)
{
 ...
 */
static PRInt32
pt_AIXDispatchSendFile(PRFileDesc* sd, PRSendFileData* sfd,
...
static PRInt32
pt_AIXSendFile(PRFileDesc* sd, PRSendFileData* sfd,
               PRTransmitFileFlags flags,
               PRIntervalTime timeout)
{
...
static PRInt32
pt_HPUXSendFile(PRFileDesc* sd, PRSendFileData* sfd,
                PRTransmitFileFlags flags,
                PRIntervalTime timeout)
{
    struct stat statbuf;
...
static PRInt32
pt_SolarisSendFile(PRFileDesc* sd, PRSendFileData* sfd,
                   PRTransmitFileFlags flags,
                   PRIntervalTime timeout)
{
    struct stat statbuf;
...
static void
pt_solaris_sendfilev_init_routine(void)
{
    void* handle;
 ...
 */
static PRInt32
pt_SolarisDispatchSendFile(PRFileDesc* sd, PRSendFileData* sfd,
                           PRTransmitFileFlags flags,
                           PRIntervalTime timeout)
{
    int rv;
...
static PRInt32
pt_LinuxSendFile(PRFileDesc* sd, PRSendFileData* sfd,
                 PRTransmitFileFlags flags,
                 PRIntervalTime timeout)
{
    struct stat statbuf;
...
static PRInt32
pt_SendFile(PRFileDesc* sd, PRSendFileData* sfd,
...
static PRInt32
pt_TransmitFile(PRFileDesc* sd, PRFileDesc* fd,
...
static PRInt32
pt_AcceptRead(PRFileDesc* sd, PRFileDesc** nd, PRNetAddr** raddr,
...
static PRStatus
pt_GetSockName(PRFileDesc* fd, PRNetAddr* addr)
{
    PRIntn rv = -1;
    pt_SockLen addr_len = sizeof(PRNetAddr);
...
static PRStatus
pt_GetPeerName(PRFileDesc* fd, PRNetAddr* addr)
{
    PRIntn rv = -1;
    pt_SockLen addr_len = sizeof(PRNetAddr);
...
static PRStatus
pt_GetSocketOption(PRFileDesc* fd, PRSocketOptionData* data)
{
    PRIntn rv;
    pt_SockLen length;
    PRInt32 level, name;
...
static PRStatus
pt_SetSocketOption(PRFileDesc* fd,
...
static PRIOMethods _pr_socketpollfd_methods = {
    ...
    defined(__GLIBC__) || defined(AIX) || defined(FREEBSD) || \
    defined(NETBSD) || defined(OPENBSD) || defined(NTO) ||    \
    defined(DARWIN) || defined(RISCOS)
...
static PRFileDesc*
pt_SetMethods(PRIntn osfd, PRDescType type,
              PRBool isAcceptedSocket, PRBool imported)
{
...
PR_EXTERN(PRStatus)
_pr_push_ipv6toipv4_layer(PRFileDesc* fd);
...
static PRInt32
_pr_poll_with_poll(PRPollDesc* pds, PRIntn npds,
                   PRIntervalTime timeout)
{
    PRInt32 ready = 0;
    /*
...
PR_IMPLEMENT(PRInt32)
PR_FileDesc2NativeHandle(PRFileDesc* bottom)
{
    PRInt32 osfd = -1;
    bottom =
...
PR_IMPLEMENT(void)
PR_FD_SET(PRFileDesc* fh, PR_fd_set* set)
{
    static PRBool unwarned = PR_TRUE;
    if (unwarned) {
...
PR_IMPLEMENT(void)
PR_FD_CLR(PRFileDesc* fh, PR_fd_set* set)
{
...
PR_IMPLEMENT(PRInt32)
PR_FD_ISSET(PRFileDesc* fh, PR_fd_set* set)
{
    PRUint32 index;
    static PRBool unwarned = PR_TRUE;
    if (unwarned) {
...
PR_IMPLEMENT(void)
PR_FD_NSET(PRInt32 fd, PR_fd_set* set)
{
    static PRBool unwarned = PR_TRUE;
    if (unwarned) {
...
PR_IMPLEMENT(void)
PR_FD_NCLR(PRInt32 fd, PR_fd_set* set)
{
    PRUint32 index, index2;
    static PRBool unwarned = PR_TRUE;
    if (unwarned) {
...
PR_IMPLEMENT(PRInt32)
PR_FD_NISSET(PRInt32 fd, PR_fd_set* set)
{
    PRUint32 index;
    static PRBool unwarned = PR_TRUE;
    if (unwarned) {
...
PR_IMPLEMENT(PRStatus)
PR_CloseDirUTF16(PRDir* dir)
{
    PR_SetError(PR_NOT_IMPLEMENTED_ERROR, 0);
    return PR_FAILURE;
}

PR_IMPLEMENT(PRDirUTF16*)
