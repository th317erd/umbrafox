...
#if _POSIX_THREAD_PRIORITY_SCHEDULING > 0
static PRIntn
pt_PriorityMap(PRThreadPriority pri)
{
#ifdef NTO
 ...
 */
static int
pt_RelativePriority(int nice, PRThreadPriority pri)
{
    return nice + (1 - pri);
}
#endif
...
*/
static void
_PR_InitializeStack(PRThreadStack* ts)
{
    if (ts && (ts->stackTop == 0)) {
...
static PRThread*
_PR_CreateThread(PRThreadType type, void (*start)(void* arg),
...
PR_IMPLEMENT(void*)
GetExecutionEnvironment(PRThread* thred)
{
    return thred->environment;
} /* GetExecutionEnvironment */

PR_IMPLEMENT(void)
SetExecutionEnvironment(PRThread* thred, void* env)
{
...
static void
_pt_thread_death_internal(void* arg, PRBool callDestructors)
{
    PRThread* thred = (PRThread*)arg;
...
PR_IMPLEMENT(void)
PR_ProcessExit(PRIntn status) { _exit(status); }

PR_IMPLEMENT(PRUint32)
PR_GetThreadID(PRThread* thred)
{
...
#if defined(PT_NO_SIGTIMEDWAIT)
static void
null_signal_handler(PRIntn sig)
{
    return;
}
#endif
...
static void
suspend_signal_handler(PRIntn sig)
{
    PRThread* me = PR_GetCurrentThread();
