...
#if defined(SOLARIS) || (defined(LINUX) && defined(_REENTRANT) && \
...
*/
static char*
Alloc(PRIntn amount, char** bufp, PRIntn* buflenp, PRIntn align)
{
    char* buf = *bufp;
...
*/
static void
MakeIPv4MappedAddr(const char* v4, char* v6)
{
    memset(v6, 0, 10);
    memset(v6 + 10, 0xff, 2);
    memcpy(v6 + 12, v4, 4);
...
*/
static void
MakeIPv4CompatAddr(const char* v4, char* v6)
{
    memset(v6, 0, 12);
    memcpy(v6 + 12, v4, 4);
}
...
*/
static PRStatus
CopyHostent(struct hostent* from, char** buf, PRIntn* bufsize,
...
*/
static PRStatus
CopyProtoent(struct protoent* from, char* buf, PRIntn bufsize,
             PRProtoEnt* to)
{
    PRIntn len, na;
    char** ap;
...
*/
static PRStatus
AppendV4AddrsToHostent(struct hostent* from, char** buf,
...
static struct protoent*
getprotobynumber_r(PRInt32 number)
{
...
PRUintn
_PR_NetAddrSize(const PRNetAddr* addr)
{
    PRUintn addrsize;
 ...
 */
static int
StringToV6Addr(const char* string, PRIPv6Addr* addr)
{
 ...
 */
static const char*
V6AddrToString(const PRIPv6Addr* addr, char* buf,
               PRUint32 size)
{
#define STUFF(c)         \
    do {                 \
...
PR_ntohs(PRUint16 n) { return ntohs(n); }
PR_IMPLEMENT(PRUint32)
PR_ntohl(PRUint32 n) { return ntohl(n); }
PR_IMPLEMENT(PRUint16)
PR_htons(PRUint16 n) { return htons(n); }
PR_IMPLEMENT(PRUint32)
PR_htonl(PRUint32 n) { return htonl(n); }
PR_IMPLEMENT(PRUint64)
PR_ntohll(PRUint64 n)
{
#ifdef IS_BIG_ENDIAN
    return n;
#else
...
static PRAddrInfo*
pr_GetAddrInfoByNameFB(const char* hostname, PRUint16 af,
                       PRIntn flags)
{
    PRStatus rv;
    PRAddrInfoFB* ai;
...
PR_GetCanonNameFromAddrInfo(const PRAddrInfo* ai)
{
#if defined(_PR_HAVE_GETADDRINFO)
#if defined(_PR_INET6_PROBE)
    if (!_pr_ipv6_is_present()) {
...
#if defined(_PR_HAVE_GETADDRINFO)
static PRStatus
pr_StringToNetAddrGAI(const char* string, PRNetAddr* addr)
{
    PRADDRINFO *res, hints;
...
static PRStatus
pr_StringToNetAddrFB(const char* string, PRNetAddr* addr)
{
    PRIntn rv;
...
#if defined(_PR_HAVE_GETADDRINFO)
static PRStatus
pr_NetAddrToStringGNI(const PRNetAddr* addr, char* string,
                      PRUint32 size)
{
    int addrlen;
...
#if !defined(_PR_HAVE_GETADDRINFO) || defined(_PR_INET6_PROBE)
static PRStatus
pr_NetAddrToStringFB(const PRNetAddr* addr, char* string,
                     PRUint32 size)
{
