...
PR_GetVersion(void) { return PR_VERSION; }
...
#ifdef WINNT
static void
_pr_SetNativeThreadsOnlyMode(void)
{
    HMODULE mainExe;
    PRBool* globalp;
    char* envp;
...
void
_PR_ImplicitInitialization(void)
{
    _PR_InitStuff();
...
#else
PR_IMPLEMENT(void)
PR_ProcessExit(PRIntn status) { _PR_MD_EXIT(status); }
...
PRBool
_PR_Obsolete(const char* obsolete, const char* preferred)
{
#if defined(DEBUG)
