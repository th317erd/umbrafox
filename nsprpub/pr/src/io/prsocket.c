...
SocketWritev(PRFileDesc* fd, const PRIOVec* iov,
             PRInt32 iov_size,
...
SocketConnect(PRFileDesc* fd, const PRNetAddr* addr,
              PRIntervalTime timeout)
{
...
static PRStatus PR_CALLBACK
SocketConnectContinue(PRFileDesc* fd,
                      PRInt16 out_flags)
{
    PROsfd osfd;
    int err;
...
SocketAccept(PRFileDesc* fd, PRNetAddr* addr,
             PRIntervalTime timeout)
{
    PROsfd osfd;
    PRFileDesc* fd2;
...
static PRStatus PR_CALLBACK
SocketBind(PRFileDesc* fd, const PRNetAddr* addr)
{
    PRInt32 result;
    const PRNetAddr* addrp = addr;
#if defined(_PR_INET6)
...
static PRStatus PR_CALLBACK
SocketListen(PRFileDesc* fd, PRIntn backlog)
{
    PRInt32 result;
...
static PRStatus PR_CALLBACK
SocketShutdown(PRFileDesc* fd, PRIntn how)
{
    PRInt32 result;
...
static PRInt32 PR_CALLBACK
SocketRecv(PRFileDesc* fd, void* buf, PRInt32 amount,
...
static PRInt32 PR_CALLBACK
SocketRead(PRFileDesc* fd, void* buf,
           PRInt32 amount)
{
...
static PRInt32 PR_CALLBACK
SocketSend(PRFileDesc* fd, const void* buf,
...
static PRInt32 PR_CALLBACK
SocketWrite(PRFileDesc* fd, const void* buf,
            PRInt32 amount)
{
...
SocketAvailable64(PRFileDesc* fd)
{
    PRInt64 rv;
#ifdef _PR_HAVE_PEEK_BUFFER
    if (fd->secret->peekBytes != 0) {
...
static PRStatus PR_CALLBACK
SocketSync(PRFileDesc* fd)
{
    return PR_SUCCESS;
}

static PRInt32 PR_CALLBACK
SocketSendTo(PRFileDesc* fd, const void* buf,
             PRInt32 amount, PRIntn flags,
...
#if defined(_WIN64) && defined(WIN95)
static PRInt32 PR_CALLBACK
SocketTCPSendTo(PRFileDesc* fd, const void* buf,
                PRInt32 amount, PRIntn flags,
                const PRNetAddr* addr,
                PRIntervalTime timeout)
{
...
static PRInt32 PR_CALLBACK
SocketRecvFrom(PRFileDesc* fd, void* buf,
               PRInt32 amount, PRIntn flags,
               PRNetAddr* addr,
               PRIntervalTime timeout)
{
...
static PRInt32 PR_CALLBACK
SocketAcceptRead(PRFileDesc* sd, PRFileDesc** nd,
...
static PRInt32 PR_CALLBACK
SocketSendFile(PRFileDesc* sd, PRSendFileData* sfd,
...
static PRInt32 PR_CALLBACK
SocketTransmitFile(PRFileDesc* sd, PRFileDesc* fd,
...
static PRStatus PR_CALLBACK
SocketGetName(PRFileDesc* fd, PRNetAddr* addr)
{
    PRInt32 result;
    PRUint32 addrlen;
...
static PRStatus PR_CALLBACK
SocketGetPeerName(PRFileDesc* fd, PRNetAddr* addr)
{
    PRInt32 result;
    PRUint32 addrlen;
...
SocketPoll(PRFileDesc* fd, PRInt16 in_flags,
           PRInt16* out_flags)
{
    *out_flags = 0;
...
PR_IMPLEMENT(PRStatus)
PR_NewTCPSocketPair(PRFileDesc* f[])
{
#ifdef XP_UNIX
    PRInt32 rv, osfd[2];
...
PR_IMPLEMENT(void)
PR_FD_NSET(PROsfd fd, PR_fd_set* set)
{
...
PR_IMPLEMENT(void)
PR_FD_NCLR(PROsfd fd, PR_fd_set* set)
{
    PRUint32 index, index2;
...
PR_IMPLEMENT(PRInt32)
PR_FD_NISSET(PROsfd fd, PR_fd_set* set)
{
    PRUint32 index;
    for (index = 0; index < set->nsize; index++)
        if (set->narray[index] == fd) {
...
static PRPollDesc*
_pr_setfd(PR_fd_set* set, PRInt16 flags,
          PRPollDesc* polldesc)
{
    PRUintn fsidx, pdidx;
