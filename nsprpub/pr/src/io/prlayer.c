...
pl_DefRead(PRFileDesc* fd, void* buf,
           PRInt32 amount)
{
    PR_ASSERT(fd != NULL);
    PR_ASSERT(fd->lower != NULL);
...
static PRInt32 PR_CALLBACK
pl_DefWrite(PRFileDesc* fd, const void* buf,
            PRInt32 amount)
{
    PR_ASSERT(fd != NULL);
    PR_ASSERT(fd->lower != NULL);
...
pl_DefAvailable64(PRFileDesc* fd)
{
    PR_ASSERT(fd != NULL);
    PR_ASSERT(fd->lower != NULL);
...
static PRInt32 PR_CALLBACK
pl_DefSeek(PRFileDesc* fd, PRInt32 offset,
           PRSeekWhence how)
{
    PR_ASSERT(fd != NULL);
    PR_ASSERT(fd->lower != NULL);
...
static PRInt64 PR_CALLBACK
pl_DefSeek64(PRFileDesc* fd, PRInt64 offset,
             PRSeekWhence how)
{
    PR_ASSERT(fd != NULL);
    PR_ASSERT(fd->lower != NULL);
...
static PRStatus PR_CALLBACK
pl_DefFileInfo(PRFileDesc* fd, PRFileInfo* info)
{
    PR_ASSERT(fd != NULL);
    PR_ASSERT(fd->lower != NULL);
...
static PRStatus PR_CALLBACK
pl_DefFileInfo64(PRFileDesc* fd,
                 PRFileInfo64* info)
{
    PR_ASSERT(fd != NULL);
    PR_ASSERT(fd->lower != NULL);
...
static PRInt32 PR_CALLBACK
pl_DefWritev(PRFileDesc* fd, const PRIOVec* iov,
...
static PRStatus PR_CALLBACK
pl_DefConnect(PRFileDesc* fd, const PRNetAddr* addr,
...
static PRStatus PR_CALLBACK
pl_DefConnectcontinue(PRFileDesc* fd,
                      PRInt16 out_flags)
{
    PR_ASSERT(fd != NULL);
    PR_ASSERT(fd->lower != NULL);
...
pl_TopAccept(PRFileDesc* fd, PRNetAddr* addr,
             PRIntervalTime timeout)
{
    PRStatus rv;
...
static PRStatus PR_CALLBACK
pl_DefBind(PRFileDesc* fd, const PRNetAddr* addr)
{
    PR_ASSERT(fd != NULL);
    PR_ASSERT(fd->lower != NULL);
...
static PRStatus PR_CALLBACK
pl_DefListen(PRFileDesc* fd, PRIntn backlog)
{
    PR_ASSERT(fd != NULL);
    PR_ASSERT(fd->lower != NULL);
...
static PRStatus PR_CALLBACK
pl_DefShutdown(PRFileDesc* fd, PRIntn how)
{
    PR_ASSERT(fd != NULL);
    PR_ASSERT(fd->lower != NULL);
...
static PRInt32 PR_CALLBACK
pl_DefRecv(PRFileDesc* fd, void* buf, PRInt32 amount,
...
static PRInt32 PR_CALLBACK
pl_DefSend(PRFileDesc* fd, const void* buf,
...
static PRInt32 PR_CALLBACK
pl_DefRecvfrom(PRFileDesc* fd, void* buf,
               PRInt32 amount, PRIntn flags,
               PRNetAddr* addr,
               PRIntervalTime timeout)
{
...
static PRInt32 PR_CALLBACK
pl_DefSendto(PRFileDesc* fd, const void* buf,
             PRInt32 amount, PRIntn flags,
...
pl_DefPoll(PRFileDesc* fd, PRInt16 in_flags,
           PRInt16* out_flags)
{
    PR_ASSERT(fd != NULL);
    PR_ASSERT(fd->lower != NULL);
...
static PRInt32 PR_CALLBACK
pl_DefAcceptread(PRFileDesc* sd, PRFileDesc** nd,
...
static PRInt32 PR_CALLBACK
pl_DefTransmitfile(PRFileDesc* sd, PRFileDesc* fd,
...
static PRStatus PR_CALLBACK
pl_DefGetsockname(PRFileDesc* fd, PRNetAddr* addr)
{
    PR_ASSERT(fd != NULL);
    PR_ASSERT(fd->lower != NULL);
...
static PRStatus PR_CALLBACK
pl_DefGetpeername(PRFileDesc* fd, PRNetAddr* addr)
{
    PR_ASSERT(fd != NULL);
    PR_ASSERT(fd->lower != NULL);
...
static PRStatus PR_CALLBACK
pl_DefGetsocketoption(PRFileDesc* fd,
...
static PRInt32 PR_CALLBACK
pl_DefSendfile(PRFileDesc* sd, PRSendFileData* sfd,
               PRTransmitFileFlags flags,
               PRIntervalTime timeout)
{
    PR_ASSERT(sd != NULL);
...
PR_IMPLEMENT(PRFileDesc*)
PR_CreateIOLayer(PRFileDesc* top)
{
    PRFileDesc* fd = NULL;
...
static PRStatus
_PR_DestroyIOLayer(PRFileDesc* stack)
{
    if (NULL == stack) {
        return PR_FAILURE;
    }
...
PR_IMPLEMENT(PRFileDesc*)
PR_PopIOLayer(PRFileDesc* stack, PRDescIdentity id)
{
