...
FileRead(PRFileDesc* fd, void* buf, PRInt32 amount)
{
    PRInt32 rv = 0;
...
static PRInt32 PR_CALLBACK
FileWrite(PRFileDesc* fd, const void* buf,
          PRInt32 amount)
{
    PRInt32 rv = 0;
    PRInt32 temp, count;
...
static PROffset32 PR_CALLBACK
FileSeek(PRFileDesc* fd, PROffset32 offset,
         PRSeekWhence whence)
{
    PROffset32 result;
...
static PROffset64 PR_CALLBACK
FileSeek64(PRFileDesc* fd, PROffset64 offset,
           PRSeekWhence whence)
{
    PROffset64 result;
...
PipeSync(PRFileDesc* fd)
{
    return PR_SUCCESS;
}

static PRStatus PR_CALLBACK
FileGetInfo(PRFileDesc* fd, PRFileInfo* info)
{
    PRInt32 rv;
...
static PRStatus PR_CALLBACK
FileGetInfo64(PRFileDesc* fd, PRFileInfo64* info)
{
    /* $$$$ NOT YET IMPLEMENTED */
    PRInt32 rv;
...
FilePoll(PRFileDesc* fd, PRInt16 in_flags,
         PRInt16* out_flags)
{
    *out_flags = 0;
    return in_flags;
