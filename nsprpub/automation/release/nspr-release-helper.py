...
o = OptionParser(
    usage="client.py [options] remove_beta | set_beta | print_library_versions | set_version_to_minor_release | set_version_to_patch_release | create_nspr_release_archive"
)
...
def check_files_exist():
    if not os.path.exists(prinit_h):
        exit_with_failure(
            "cannot find expected header files, must run from inside NSPR git directory"
        )
...
def toggle_beta_status(is_beta):
    ...
    print(
        "please run 'git status' and 'git diff' to verify the files have been verified correctly"
    )
...
def print_library_versions():
    ...
    check_call_noisy(
        [
            "egrep",
            "#define *PR_VERSION|#define PR_VMAJOR|#define *PR_VMINOR|#define *PR_VPATCH|#define *PR_BETA",
            prinit_h,
        ]
    )
