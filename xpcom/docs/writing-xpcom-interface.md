...
Often these XPCOM components are wired into the {code}`Services` JavaScript object to allow for ergonomic access to the interface. For example, open the [Browser Console] and type {code}`Services.` to interactively access these components.
...
It is probably worth reading over {ref}`defining-xpcom-components`, but the following config will be sufficient to hook up our component to the {code}`Services` object.
