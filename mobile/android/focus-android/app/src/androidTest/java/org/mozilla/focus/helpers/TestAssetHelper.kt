...
object TestAssetHelper {
    ...
    /**
     * Hosts simple websites, found at androidTest/assets/tab[1|2|3].html Returns a list of TestAsset, which can be used
     * to navigate to each and assert that the correct information is being displayed.
     *
     * Content for these pages all follow the same pattern. See [tab1.html] for content implementation details.
     */
    fun MockWebServer.getGenericTabAsset(pageNum: Int) =
        createTestAsset(
    ...
    fun MockWebServer.getEnhancedTrackingProtectionAsset(pageTitle: String) =
        createTestAsset(
    ...
    fun MockWebServer.getMediaTestAsset(pageTitle: String) =
        createTestAsset(
            path = "$pageTitle.html",
            title = pageTitle,
        )
