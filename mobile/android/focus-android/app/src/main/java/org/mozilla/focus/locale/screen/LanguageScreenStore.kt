 ...
 *
 * This store is responsible for handling the state ([LanguageScreenState]) and actions ([LanguageScreenAction]) related
 * to the language selection UI. It manages the list of available languages and the currently selected language.
 *
 * On initialization, it automatically dispatches [LanguageScreenAction.InitLanguages]. This action is intended to be
 * intercepted by a [Middleware] to trigger the loading of language data from a repository.
 *
 * @param initialState The initial state for the language screen.
 * @param middlewares A list of [Middleware]s to intercept actions and perform side effects, such as fetching the
 *   language list.
 */
...
sealed class LanguageScreenAction : Action {
    ...
    /**
     * An action to update the entire list of available languages and the currently selected language.
     *
