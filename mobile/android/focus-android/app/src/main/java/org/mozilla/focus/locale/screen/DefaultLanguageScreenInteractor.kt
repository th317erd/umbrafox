...
/**
 * The default implementation of the interactor for the language selection screen. This class is responsible for
 * handling user interactions and communicating them to the store.
 *
 * @param languageScreenStore The store responsible for managing the state of the language screen.
 */
class DefaultLanguageScreenInteractor(private val languageScreenStore: LanguageScreenStore) {

    /**
     * Handles the selection of a language from the list.
     *
     * If the selected language is the same as the one already selected, it does nothing. Otherwise, it dispatches an
     * action to the store to update the selected language.
     *
