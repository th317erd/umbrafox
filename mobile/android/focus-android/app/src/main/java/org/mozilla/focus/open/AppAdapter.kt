...
class AppAdapter(context: Context, infoArray: Array<ActivityInfo>, store: ActivityInfo?) :
    ...
    private val apps: List<App> =
        infoArray.map { App(context, it) }.sortedWith { app1, app2 -> app1.label.compareTo(app2.label) }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        if (viewType == AppViewHolder.LAYOUT_ID) {
            return AppViewHolder(inflater.inflate(AppViewHolder.LAYOUT_ID, parent, false))
        } else if (viewType == InstallBannerViewHolder.LAYOUT_ID) {
            return InstallBannerViewHolder(inflater.inflate(InstallBannerViewHolder.LAYOUT_ID, parent, false))
        }
