...
class MediaDelegateTest : BaseSessionTest() {
            ...
            object : MediaDelegate {
                ...
                override fun onRecordingStatusChanged(
                    ...
                    for (device in devices) {
                        if (
                            device.type ==
                                org.mozilla.geckoview.GeckoSession.MediaDelegate.RecordingDevice.Type.MICROPHONE
                        ) {
                            audioActive =
                                device.status !=
                                    org.mozilla.geckoview.GeckoSession.MediaDelegate.RecordingDevice.Status.INACTIVE
                        }
                        if (
                            device.type == org.mozilla.geckoview.GeckoSession.MediaDelegate.RecordingDevice.Type.CAMERA
                        ) {
                            cameraActive =
                                device.status !=
                                    org.mozilla.geckoview.GeckoSession.MediaDelegate.RecordingDevice.Status.INACTIVE
    ...
    @Test
    fun testDeviceRecordingEventAudio() {
        // disable test on debug Bug 1555656
        assumeThat(sessionRule.env.isDebugBuild, Matchers.equalTo(false))
        mainSession.loadTestPath(HELLO_HTML_PATH)
        mainSession.waitForPageStop()

        val devices = mainSession.waitForJS("window.navigator.mediaDevices.enumerateDevices()").asJSList<JSONObject>()
    ...
    @Test
    fun testDeviceRecordingEventVideo() {
        mainSession.loadTestPath(HELLO_HTML_PATH)
        mainSession.waitForPageStop()
    ...
    @Test
    fun testDeviceRecordingEventAudioAndVideo() {
