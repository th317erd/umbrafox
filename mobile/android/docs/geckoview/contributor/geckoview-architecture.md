...
myst:
  substitutions:
    ...
    code_layers: |-
      ```{image} ../assets/code-layers.png```
    pageload_diagram: |-
      ```{image} ../assets/pageload-diagram.png```
    view_runtime_session: |-
      ```{image} ../assets/view-runtime-session.png```
---

(geckoview-architecture-overview)=
...
- `TextureView` offers a surface which can be transformed and animated but
  it's slower and requires more memory because it's [triple-buffered](https://en.wikipedia.org/wiki/Multiple_buffering#Triple_buffering)
...
the Android CA store by setting the [enterpriseRootsEnabled](<https://mozilla.github.io/geckoview/javadoc/mozilla-central/org/mozilla/geckoview/GeckoRuntimeSettings.Builder.html#enterpriseRootsEnabled(boolean)>)
...
roots, although this might change with [Bug 1522162](https://bugzilla.mozilla.org/show_bug.cgi?id=1522162).
...
Currently, the [Glean](https://docs.telemetry.mozilla.org/concepts/glean/glean.html) library is
...
file for [geckoview-omni](https://maven.mozilla.org/maven2/org/mozilla/geckoview/geckoview-omni/102.0.20220623063721/geckoview-omni-102.0.20220623063721.module):
...
engineers](https://gist.github.com/agi/c900f3e473ff681158c0c907e34780e4).
...
process have `active=true`. See also [GeckoDisplay].
...
priority, [setPriorityHint](<https://mozilla.github.io/geckoview/javadoc/mozilla-central/org/mozilla/geckoview/GeckoSession.html#setPriorityHint(int)>).
...
by principal. See also [Setting Permissions].

To learn more about principals see [this talk by Bobby Holley](https://www.youtube.com/watch?v=28FPetl5Fl4).
...
[principals]) and are managed internally using `nsIPermissionManager`.
...
You can read more about Android permissions on [this doc](https://developer.android.com/guide/topics/permissions/overview).
...
Permissions are stored in a map between a [principal](#principals) and a list
...
consumers to get hold of the [ContentPermission](https://mozilla.github.io/geckoview/javadoc/mozilla-central/org/mozilla/geckoview/GeckoSession.PermissionDelegate.ContentPermission.html)
object. The ContentPermission object is returned in [onLocationChange](<https://mozilla.github.io/geckoview/javadoc/mozilla-central/org/mozilla/geckoview/GeckoSession.NavigationDelegate.html#onLocationChange(org.mozilla.geckoview.GeckoSession,java.lang.String,java.util.List)>)
...
GeckoView supports third-party autofill providers through Android's [autofill framework](https://developer.android.com/guide/topics/text/autofill). Internally, this support is referred to as `autofill`.
...
detailed description of this see [here](https://gist.github.com/agi/c900f3e473ff681158c0c907e34780e4#actors).
