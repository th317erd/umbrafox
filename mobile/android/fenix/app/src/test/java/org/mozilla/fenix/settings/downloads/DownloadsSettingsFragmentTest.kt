...
class DownloadsSettingsFragmentTest {
    private lateinit var fragment: DownloadsSettingsFragment

    @Before
    fun setUp() {
        every { testContext.components.settings } returns mockk(relaxed = true)

        fragment = DownloadsSettingsFragment()

        val activity = Robolectric.buildActivity(FragmentActivity::class.java).create().get()
        activity.supportFragmentManager.beginTransaction().add(fragment, "DownloadsSettingsFragmentTest").commitNow()
    }
    ...
    fun `GIVEN no activity can handle the folder picker WHEN launching the directory picker THEN a toast is shown and no exception propagates`() {
        fragment.launcher = mockk { every { launch(any()) } throws ActivityNotFoundException() }
