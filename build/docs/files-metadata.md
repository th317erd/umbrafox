...
`Files` metadata is extracted in {ref}`mozbuild-fs-reading-mode`.
...
{ref}`mozbuild-fs-reading-mode`, with the set of evaluated `moz.build`
...
(mozbuild-files-metadata-finalizing)=
