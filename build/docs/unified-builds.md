...
called "[unity builds](https://en.wikipedia.org/wiki/Unity_build)") to
...
(unified-build-compilation-failures)=
...
- The amount of files per chunk can be adjusted in `moz.build` files with the
  ...
  compilation failures as described {ref}`above<unified-build-compilation-failures>`.
