...
RuleAction = Union[LoadFileAction, FetchRevisionAction]
...
def _parse_bugzilla_predicate(value: object, path: str) -> BugzillaPredicate:
    table = _require_table(value, path)
    _reject_unknown_keys(path, table, {"component", "ref"})
    if "ref" in table:
        if len(table) != 1:
            raise ReviewContextValidationError(
                f"{path}.ref: cannot combine with fields"
...
def _parse_review_predicate(value: object, path: str) -> ReviewPredicate:
    table = _require_table(value, path)
    _reject_unknown_keys(path, table, {"reviewer", "blocking_reviewer", "ref"})
    if "ref" in table:
        if len(table) != 1:
            raise ReviewContextValidationError(
                f"{path}.ref: cannot combine with fields"
    ...
    if not predicate.reviewer and not predicate.blocking_reviewer:
        raise ReviewContextValidationError(f"{path}: expected at least one field")
    return predicate
...
PredicateParser = Callable[[object, str, Definitions], Predicate]
