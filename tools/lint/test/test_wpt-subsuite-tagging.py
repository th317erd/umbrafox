...
def test_dir_tag_passes(lint, paths):
    results = lint(paths("testing/web-platform/tests/webgpu-with-dir-tag/test.html"))
    assert len(results) == 0


def test_file_tag_passes(lint, paths):
    results = lint(paths("testing/web-platform/tests/webgpu-with-file-tag/test.html"))
    assert len(results) == 0


def test_variant_tag_passes(lint, paths):
    results = lint(
        paths("testing/web-platform/tests/webgpu-with-variant-tag/test.html")
    )
    assert len(results) == 0


def test_untagged_errors(lint, paths):
    results = lint(paths("testing/web-platform/tests/webgpu-untagged/test.html"))
    assert len(results) == 1
    assert "webgpu" in results[0].message


def test_not_subsuite_passes(lint, paths):
    results = lint(paths("testing/web-platform/tests/not-subsuite-webgpu/test.html"))
    assert len(results) == 0
