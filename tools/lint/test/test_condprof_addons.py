...
def linter_module_mocks(
    linter_module,
    customizations_path=".",
    browsertime_fetches_path="browsertime.yml",
    **othermocks,
):
...
def linter_class_mocks(linter_module, **mocks):
    return mock.patch.multiple(
        linter_module.CondprofAddonsLinter,
        **mocks,
    )


# Sanity check (make sure linter message includes the xpi filename).
def test_get_missing_xpi_msg(lint, paths, linter_module):
    with linter_class_mocks(
        linter_module,
        get_firefox_addons_tar_names=mock.Mock(return_value=list()),
    ):
        instance = linter_module.CondprofAddonsLinter(
...
def test_xpi_missing_from_firefox_addons_tar(lint, paths, linter_module):
    fixture_customizations = paths("with-missing-xpi.json")
    with linter_module_mocks(linter_module), linter_class_mocks(
        linter_module,
        get_firefox_addons_tar_names=mock.Mock(return_value=list()),
    ):
...
def test_xpi_all_found_in_firefox_addons_tar(lint, paths, linter_module):
...
def test_lint_error_on_missing_or_invalid_firefoxaddons_fetch_task(
    ...
    with linter_module_mocks(linter_module), linter_class_mocks(
        linter_module, read_json=read_json_mock, read_yaml=read_yaml_mock
    ):
        logger_mock = mock.Mock()
        fixture_customizations = paths("fake-condprof-config.json")
        ...
        def assert_linter_error(yaml_mock_value, expected_msg):
            ...
            lint(fixture_customizations, logger=logger_mock)
            assert logger_mock.lint_error.call_count == 1
            expected_path = linter_module.BROWSERTIME_FETCHES_PATH
        ...
        assert_linter_error(
            ...
            expected_msg=linter_module.ERR_FETCH_TASK_MISSING,
        )
        ...
        assert_linter_error(
            ...
            expected_msg=linter_module.ERR_FETCH_TASK_ADDPREFIX,
        )
...
def test_get_xpi_list_from_fetch_dir(lint, paths, linter_module):
    ...
    with linter_module_mocks(
        linter_module, MOZ_AUTOMATION=1, MOZ_FETCHES_DIR=paths("fake-fetches-dir")[0]
    ):
        logger_mock = mock.Mock()
        Path(paths("browsertime.yml")[0])

        linter = linter_module.CondprofAddonsLinter(
...
def test_get_xpi_list_from_downloaded_tar(lint, paths, linter_module):
...
def test_error_on_downloading_tar(requests_get_mock, lint, paths, linter_module):
    ...
    with tempfile.TemporaryDirectory() as tempdir, linter_module_mocks(
        linter_module, MOZ_AUTOMATION=0, tempdir=tempdir
    ):
        logger_mock = mock.Mock()
        ...
        assert (
            logger_mock.lint_error.call_args.kwargs["path"]
            == linter_module.BROWSERTIME_FETCHES_PATH
        )
        assert (
            logger_mock.lint_error.call_args.args[0]
            == f"{linter_module.ERR_FETCH_TASK_ARCHIVE}, MOCK_ERROR"
        )
...
@mock.patch("requests.get")
def test_error_on_opening_tar(requests_get_mock, lint, paths, linter_module):
    ...
    with tempfile.TemporaryDirectory() as tempdir, linter_module_mocks(
        ...
        assert actual_msg.startswith(
            f"{linter_module.ERR_FETCH_TASK_ARCHIVE}, file could not be opened successfully"
        )
        assert requests_get_mock.call_count == 1
        assert len(linter.tar_xpi_filenames) == 0
