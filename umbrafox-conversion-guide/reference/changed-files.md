# Changed files inventory

This inventory was generated from the current working tree when the guide was written. It should be refreshed when the conversion is committed or expanded.

## Core app identity and startup/default prefs

- `browser/app/profile/firefox.js`
- `browser/moz.configure`
- `build/moz.configure/init.configure`
- `toolkit/xre/nsAppRunner.cpp`
- `tools/@types/subs/AppConstants.sys.d.mts`
- `modules/libpref/init/all.js`

## Branding framework and channel metadata

- `browser/branding/branding-common.mozbuild`
- `browser/branding/aurora/branding.nsi`
- `browser/branding/aurora/configure.sh`
- `browser/branding/aurora/moz.build`
- `browser/branding/nightly/branding.nsi`
- `browser/branding/nightly/configure.sh`
- `browser/branding/nightly/moz.build`
- `browser/branding/official/branding.nsi`
- `browser/branding/official/configure.sh`
- `browser/branding/official/moz.build`
- `browser/branding/unofficial/branding.nsi`
- `browser/branding/unofficial/configure.sh`
- `browser/branding/unofficial/moz.build`

## Branding strings

- `browser/branding/aurora/locales/en-US/brand.ftl`
- `browser/branding/aurora/locales/en-US/brand.properties`
- `browser/branding/nightly/locales/en-US/brand.ftl`
- `browser/branding/nightly/locales/en-US/brand.properties`
- `browser/branding/official/locales/en-US/brand.ftl`
- `browser/branding/official/locales/en-US/brand.properties`
- `browser/branding/unofficial/locales/en-US/brand.ftl`
- `browser/branding/unofficial/locales/en-US/brand.properties`

## Branding visual assets

- `browser/branding/aurora/content/about-wordmark.svg`
- `browser/branding/aurora/content/firefox-wordmark.svg`
- `browser/branding/nightly/content/about-wordmark.svg`
- `browser/branding/nightly/content/firefox-wordmark.svg`
- `browser/branding/official/content/about-wordmark.svg`
- `browser/branding/official/content/firefox-wordmark.svg`
- `browser/branding/unofficial/PrivateBrowsing_150.png`
- `browser/branding/unofficial/PrivateBrowsing_70.png`
- `browser/branding/unofficial/VisualElements_150.png`
- `browser/branding/unofficial/VisualElements_70.png`
- `browser/branding/unofficial/content/about-logo-private.png`
- `browser/branding/unofficial/content/about-logo-private@2x.png`
- `browser/branding/unofficial/content/about-logo.png`
- `browser/branding/unofficial/content/about-logo.svg`
- `browser/branding/unofficial/content/about-logo@2x.png`
- `browser/branding/unofficial/content/about-wordmark.svg`
- `browser/branding/unofficial/content/about.png`
- `browser/branding/unofficial/content/firefox-wordmark.svg`
- `browser/branding/unofficial/default128.png`
- `browser/branding/unofficial/default16.png`
- `browser/branding/unofficial/default22.png`
- `browser/branding/unofficial/default24.png`
- `browser/branding/unofficial/default256.png`
- `browser/branding/unofficial/default32.png`
- `browser/branding/unofficial/default48.png`
- `browser/branding/unofficial/default64.png`
- `browser/branding/unofficial/document.ico`
- `browser/branding/unofficial/document_pdf.ico`
- `browser/branding/unofficial/firefox.ico`
- `browser/branding/unofficial/firefox64.ico`
- `browser/branding/unofficial/newtab.ico`
- `browser/branding/unofficial/newwindow.ico`
- `browser/branding/unofficial/pbmode.ico`

## New tab, Home, and Settings UI

- `browser/extensions/newtab/content-src/components/Base/Base.jsx`
- `browser/extensions/newtab/lib/ActivityStream.sys.mjs`
- `browser/extensions/newtab/lib/AboutPreferences.sys.mjs`
- `browser/components/preferences/home.inc.xhtml`
- `browser/components/preferences/config/search.mjs`
- `browser/components/preferences/config/permissions-data.mjs`

## Search engine policy

- `services/settings/dumps/main/search-config-v2.json`
- `services/settings/dumps/main/search-config-overrides-v2.json`
- `services/settings/dumps/main/search-default-override-allowlist.json`
- `toolkit/components/search/SearchEngineSelector.sys.mjs`
- `toolkit/components/search/tests/xpcshell/test_engine_selector_remote_settings.js`

## New tab and defaults tests

- `browser/components/newtab/test/xpcshell/xpcshell.toml`
- `browser/components/newtab/test/xpcshell/test_UmbrafoxHomeDefaults.js`
- `browser/extensions/newtab/test/browser/browser_activation_window.js`
- `browser/extensions/newtab/test/browser/browser_customize_menu_content.js`
- `browser/extensions/newtab/test/browser/browser_customize_menu_key_open.js`
- `browser/extensions/newtab/test/browser/browser_customize_menu_render.js`

## Browser en-US localization

- `browser/locales/en-US/browser/aboutDialog.ftl`
- `browser/locales/en-US/browser/aboutLogins.ftl`
- `browser/locales/en-US/browser/aboutRobots.ftl`
- `browser/locales/en-US/browser/aboutUnloads.ftl`
- `browser/locales/en-US/browser/accounts.ftl`
- `browser/locales/en-US/browser/addonNotifications.ftl`
- `browser/locales/en-US/browser/aiWindow.ftl`
- `browser/locales/en-US/browser/appExtensionFields.ftl`
- `browser/locales/en-US/browser/appmenu.ftl`
- `browser/locales/en-US/browser/backupSettings.ftl`
- `browser/locales/en-US/browser/browser.ftl`
- `browser/locales/en-US/browser/browserContext.ftl`
- `browser/locales/en-US/browser/browserSets.ftl`
- `browser/locales/en-US/browser/contextual-manager.ftl`
- `browser/locales/en-US/browser/featureCallout.ftl`
- `browser/locales/en-US/browser/firefoxView.ftl`
- `browser/locales/en-US/browser/fxviewTabList.ftl`
- `browser/locales/en-US/browser/ipProtection.ftl`
- `browser/locales/en-US/browser/linuxDesktopEntry.ftl`
- `browser/locales/en-US/browser/menubar.ftl`
- `browser/locales/en-US/browser/migrationWizard.ftl`
- `browser/locales/en-US/browser/newtab/asrouter.ftl`
- `browser/locales/en-US/browser/newtab/newtab.ftl`
- `browser/locales/en-US/browser/newtab/onboarding.ftl`
- `browser/locales/en-US/browser/policies/policies-descriptions.ftl`
- `browser/locales/en-US/browser/preferences/moreFromMozilla.ftl`
- `browser/locales/en-US/browser/preferences/preferences.ftl`
- `browser/locales/en-US/browser/profile/default-bookmarks.ftl`
- `browser/locales/en-US/browser/profiles.ftl`
- `browser/locales/en-US/browser/protections.ftl`
- `browser/locales/en-US/browser/siteProtections.ftl`
- `browser/locales/en-US/browser/spotlight.ftl`
- `browser/locales/en-US/browser/translations.ftl`
- `browser/locales/en-US/chrome/browser/browser.properties`
- `browser/locales/en-US/chrome/browser/downloads/downloads.properties`
- `browser/locales/en-US/chrome/overrides/appstrings.properties`
- `browser/locales/en-US/updater/updater.ini`

## DevTools en-US localization

- `devtools/client/locales/en-US/aboutdebugging.ftl`
- `devtools/client/locales/en-US/application.ftl`
- `devtools/client/locales/en-US/compatibility.ftl`
- `devtools/client/locales/en-US/debugger.properties`
- `devtools/client/locales/en-US/jsonview.properties`
- `devtools/client/locales/en-US/memory.properties`
- `devtools/client/locales/en-US/responsive.properties`
- `devtools/client/locales/en-US/sourceeditor.properties`
- `devtools/client/locales/en-US/toolbox.ftl`
- `devtools/client/locales/en-US/toolbox.properties`

## Toolkit en-US localization and support files

- `toolkit/content/aboutTelemetry.xhtml`
- `toolkit/locales/en-US/chrome/global/aboutStudies.properties`
- `toolkit/locales/en-US/chrome/global/narrate.properties`
- `toolkit/locales/en-US/toolkit/about/aboutAddons.ftl`
- `toolkit/locales/en-US/toolkit/about/aboutCompat.ftl`
- `toolkit/locales/en-US/toolkit/about/aboutProcesses.ftl`
- `toolkit/locales/en-US/toolkit/about/aboutSupport.ftl`
- `toolkit/locales/en-US/toolkit/about/aboutTelemetry.ftl`
- `toolkit/locales/en-US/toolkit/branding/brandings.ftl`
- `toolkit/locales/en-US/toolkit/firefoxlabs/features.ftl`
- `toolkit/locales/en-US/toolkit/formautofill/formAutofill.ftl`
- `toolkit/locales/en-US/toolkit/global/commonDialog.ftl`
- `toolkit/locales/en-US/toolkit/global/contextual-identity.ftl`
- `toolkit/locales/en-US/toolkit/global/textActions.ftl`
- `toolkit/locales/en-US/toolkit/pdfviewer/viewer.ftl`
- `tools/lint/fluent-lint/exclusions.yml`

## Userland scripts

- `browser/base/content/aboutUmbrafoxUserland.xhtml`
- `browser/base/content/aboutUmbrafoxUserland.css`
- `browser/base/jar.mn`
- `browser/components/about/AboutRedirector.cpp`
- `browser/components/about/components.conf`
- `browser/locales/en-US/browser/aboutUmbrafoxUserland.ftl`
- `devtools/client/debugger/src/actions/context-menus/source-tree-item.js`
- `devtools/client/debugger/src/client/firefox.js`
- `devtools/client/debugger/src/client/firefox/create.js`
- `devtools/client/debugger/src/components/App.js`
- `devtools/client/debugger/src/components/Editor/index.js`
- `devtools/client/debugger/src/components/PrimaryPanes/SourcesTree.js`
- `devtools/client/debugger/src/components/PrimaryPanes/SourcesTreeItem.js`
- `devtools/client/debugger/src/reducers/sources-tree.js`
- `devtools/client/debugger/src/selectors/sources-tree.js`
- `devtools/client/debugger/src/utils/moz.build`
- `devtools/client/debugger/src/utils/umbrafox-userland-scripts.js`
- `docs/config.yml`
- `docshell/base/nsDocShell.cpp`
- `docshell/base/nsDocShell.h`
- `docshell/base/nsDocShellLoadState.cpp`
- `docshell/base/nsDocShellLoadState.h`
- `dom/ipc/DOMTypes.ipdlh`
- `dom/script/ModuleLoader.cpp`
- `dom/script/ScriptLoader.cpp`
- `dom/script/ScriptLoader.h`
- `caps/nsScriptSecurityManager.cpp`
- `caps/nsScriptSecurityManager.h`
- `dom/base/CharacterData.cpp`
- `dom/base/Element.cpp`
- `dom/base/UmbrafoxUserlandMutation.cpp`
- `dom/base/UmbrafoxUserlandMutation.h`
- `dom/base/moz.build`
- `dom/base/nsINode.cpp`
- `js/public/Principals.h`
- `js/src/builtin/Eval.cpp`
- `js/src/vm/JSContext.cpp`
- `js/src/vm/JSContext.h`
- `js/src/vm/JSFunction.cpp`
- `toolkit/actors/UmbrafoxUserlandChild.sys.mjs`
- `toolkit/actors/UmbrafoxUserlandParent.sys.mjs`
- `toolkit/actors/moz.build`
- `toolkit/components/moz.build`
- `toolkit/components/umbrafox/`

## Bot and local control channel

- `remote/webdriver-bidi/modules/root/umbrafox.sys.mjs`
- `remote/webdriver-bidi/modules/ModuleRegistry.sys.mjs`
- `remote/webdriver-bidi/jar.mn`
- `remote/webdriver-bidi/test/xpcshell/test_UmbrafoxModule.js`
- `remote/webdriver-bidi/test/xpcshell/xpcshell.toml`
- `toolkit/components/umbrafox/UmbrafoxControlService.sys.mjs`
- `toolkit/components/umbrafox/UmbrafoxControlInput.sys.mjs`
- `toolkit/actors/UmbrafoxControlInputChild.sys.mjs`
- `toolkit/actors/UmbrafoxControlInputParent.sys.mjs`
- `toolkit/actors/moz.build`
- `toolkit/modules/ActorManagerParent.sys.mjs`
- `toolkit/components/umbrafox/tests/browser/browser_control_service.js`
- `toolkit/components/umbrafox/tests/browser/browser_control_input.js`
- `toolkit/components/umbrafox/tests/browser/browser.toml`
- `browser/components/BrowserGlue.sys.mjs`
- `browser/app/profile/firefox.js`
- `toolkit/modules/ActorManagerParent.sys.mjs`

## Bot control channel

- `remote/webdriver-bidi/modules/root/umbrafox.sys.mjs`
- `remote/webdriver-bidi/modules/ModuleRegistry.sys.mjs`
- `remote/webdriver-bidi/jar.mn`
- `remote/webdriver-bidi/test/xpcshell/test_UmbrafoxModule.js`
- `remote/webdriver-bidi/test/xpcshell/xpcshell.toml`
